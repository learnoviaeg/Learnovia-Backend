...
class LogsFiltertionController extends Controller
{
    public function logs_filteration(Request $request)
    {
        ...
        if ($user_id == null && $action == null && $start_date == null && $end_date == null && $model == null) {
            ...
            $data = AuditLog::where('created_at', '>=', $yesterday)->where('created_at', '<=', $right_now)
                                                              ->orderBy('created_at', 'DESC')->select('id', 'action','subject_type', 'subject_id', 'user_id', 'created_at', 'host')//->paginate($pagination);
                                                                ->paginate($pagination);
    	}
        ...
        if ($user_id != null && $action == null && $model == null && $start_date == null && $end_date == null) {
            ...
            $data = AuditLog::where('user_id', $user_id)->orderBy('created_at', 'DESC')->select('id', 'action','subject_type', 'subject_id', 'user_id', 'created_at', 'host')->paginate($pagination);
        }
        ...
        if ($user_id == null && $action != null && $model == null && $start_date == null && $end_date == null) {
            ...
            $data = AuditLog::where('action', $action)->orderBy('created_at', 'DESC')->select('id', 'action','subject_type', 'subject_id', 'user_id', 'created_at', 'host')->paginate($pagination);
    	}
        ...
        if ($user_id == null && $action == null && $model != null && $start_date == null && $end_date == null) {
            ...
            $data = AuditLog::where('subject_type', $model)->orderBy('created_at', 'DESC')
                                               ->select('id', 'action','subject_type', 'subject_id', 'user_id', 'created_at', 'host')->paginate($pagination);
        }
        ...
        return $this->chain_filteration($data, $year_id, $type_id, $level_id, $class_id, $segment_id, $course_id);
        // return response()->json(['data' => $data, 'status_code' => 200], 200);
    }

    public function chain_filteration($data, $year_id, $type_id, $level_id, $class_id, $segment_id, $course_id)
    {
        // filter with none
        if ($year_id == null && $type_id == null && $level_id == null && $class_id == null && $segment_id == null && $course_id == null) {
            return $this->filter_with_none($data);
        }
        // case serach with year
        elseif ($year_id != null && $type_id == null && $level_id == null && $class_id == null && $segment_id == null && $course_id == null) {
            return $this->filter_with_year($data, $year_id);
        }
        // case serach with type
        elseif ($year_id == null && $type_id != null && $level_id == null && $class_id == null && $segment_id == null && $course_id == null) {
            return $this->filter_with_type($data, $type_id);
        }
        // case serach with level
        elseif ($year_id == null && $type_id == null && $level_id != null && $class_id == null && $segment_id == null && $course_id == null) {
            return $this->filter_with_level($data, $level_id);
        }
        // case serach with course
        elseif ($year_id == null && $type_id == null && $level_id == null && $class_id == null && $segment_id == null && $course_id != null) {
            return $this->filter_with_course($data, $course_id);
        }
        // case serach with class
        elseif ($year_id == null && $type_id == null && $level_id == null && $class_id != null && $segment_id == null && $course_id == null) {
            return $this->filter_with_class($data, $class_id);
        }
        // case serach with segment
        elseif ($year_id == null && $type_id == null && $level_id == null && $class_id == null && $segment_id != null && $course_id == null) {
            return $this->filter_with_segment($data, $segment_id);
        }
        else{
            return 'another';
        }
    ...
    public function filter_with_none($data)
    {
        return response()->json(['data' => $data, 'status_code' => 200], 200);
    }

    // search with year
    public function filter_with_year($data, $year_id)
    {
     ...
     // search with type
    public function filter_with_type($data, $type_id)
    {
     ...
     // search with level
    public function filter_with_level($data, $level_id)
    {
    ...
    // search with class
    public function filter_with_class($data, $class_id)
    {
     ...
     // search with segment
    public function filter_with_segment($data, $segment_id)
    {
    ...
    // search with course
    public function filter_with_course($data, $course_id)
    {
