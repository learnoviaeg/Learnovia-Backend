...
class AnnouncementController extends Controller
{
    ...
    */
    ...
    public function new_user_announcements()
    {
        ...
        $all_ann = array();
        $all_ann['General Announcements'] = Announcement::where('assign', 'all')->get(['id', 'title', 'description', 'attached_file']);
        $g = 0;
        ...
        foreach ($uniq_seg as $seg) {
            ...
            $all_ann['Segment'][$s]['announcements'] = Announcement::where('segment_id', $segmentclass->segment_id)->get(['id', 'title', 'description', 'attached_file']);
            ...
            foreach ($segmentclass->classLevel as $scl) {
                ...
                $all_ann['Class'][$cl]['announcements'] = Announcement::where('class_id', $scl->class_id)->get(['id', 'title', 'description', 'attached_file']);
        ...
        foreach ($class_level_id as $cd) {
            ...
            foreach ($class_level->yearLevels as $yl) {
                ...
                $all_ann['Level'][$l]['announcements'] = Announcement::where('level_id', $yl->level_id)->get(['id', 'title', 'description', 'attached_file']);
                $year_level_id[] = $yl->id;
                $l++;
            }
        }
        ...
        foreach ($year_level_id as $yd) {
            ...
            foreach ($Year_level->yearType as $ayt) {
                ...
                $all_ann['Type'][$t]['announcements'] = Announcement::where('type_id', $ayt->academic_type_id)->get(['id', 'title', 'description', 'attached_file']);

                $yearname = AcademicYear::find($ayt->academic_year_id);
                $all_ann['Year'][$y]['name'] = $yearname->name;
                $all_ann['Year'][$y]['id'] = $yearname->id;
                $all_ann['Year'][$y]['announcements'] = Announcement::where('year_id', $ayt->academic_year_id)->get(['id', 'title', 'description', 'attached_file']);
        ...
        foreach ($courses as $cou) {
            ...
            $all_ann['Courses'][$co]['announcements'] = Announcement::where('course_id', $cou)->get(['id', 'title', 'description', 'attached_file']);
            $co++;
        }
