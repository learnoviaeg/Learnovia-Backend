        ...
        foreach ($request->Imported_file as $singlefile) {
            ...
            if ($check) {
                ...
                foreach ($request->lesson_id as $lesson) {
                    ...
                    $material=Material::select('id','restricted', 'name', 'publish_date','course_id','lesson_id')->where('item_id' ,$fileLesson->file_id)->where('lesson_id' ,$fileLesson->lesson_id)->where('type' , 'file')->first();
                    if(isset($request->users_ids))
                    {
                        CoursesHelper::giveUsersAccessToViewCourseItem($file->id, 'file', $request->users_ids);
